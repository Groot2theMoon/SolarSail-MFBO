...
N_EIG_BUCKLE = 60                            # [2026-09-24 실측 근거] subspace 요청 고유값 수
                                             ...
                                             #   [2026-09-24 변경] 100 -> 60 : 5회차(성공) 런에서 양수 모드가 2개만 나온 원인이
                                             #     'subspace 가 선형종속으로 250 -> 244 -> 2 로 붕괴' 였다. 기저가 과대하면
                                             #     준축퇴 음수 대역(약 230개가 3% 안)에서 방향이 붕괴한다는 가설로 블록을 줄인다.
                                             #     현재 음수 56개 < 60 이므로 조건은 유지된다. 아래 판독 규칙을 먼저 등록한다.
                                             #   판독 규칙(사전등록): 요청/기저 수는 solver 작업공간만 바꾸고 물리는 못 바꾼다.
                                             #     -> 양수 모드가 4개 이상 나오면 '2개'는 붕괴 아티팩트였다(성공).
                                             #     -> 다시 2개면 그 2개가 이 base state 의 실제 양수 고유값 수다(성공, 물리 결론).
                                             #     -> CONVERGED=0 이면 블록 축소가 역효과다(되돌리기: 100/250).
BUCKLE_VECTORS = 120                         # 기저 벡터 = 요청 수 x 2 (Abaqus 권장 하한). 이전 값은 250(붕괴 실측).
